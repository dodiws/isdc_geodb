...
def getBaselineStatistic(request,filterLock, flag, code, yy=None, mm=None, dd=None, rf_type=None, bring=None):
	...
	response = dict_ext()
	response['source'] = baseline = response_dashboard_baseline['source']
	...
	response.path('panels_list')['chart'] = [response['panels'][i] for i in ['pop','area','settlement','building','road','healthfacility']]
	...
	# for p in response['panels_list']:
	# 	if 'child' in p:
	# 		p['child'] = p['child'].values()
	# response.path('panels_list')['table'] = [response['panels'][i] for i in ['pop','area','settlement','building','road','healthfacility']]
